import { ScrapedData, PriceData, MetricData, PerformanceData, SEOData } from './scraper';
...
export class ConditionEvaluator {
  ...
  private extractValue(condition: Condition): any {
    ...
    switch (condition.field) {
      ...
      case 'seo_image_alt_ratio':
        ...
        return seo ? (seo.imageAltTags / Math.max(seo.totalImages, 1)) * 100 : 0;
  ...
  private compareValues(condition: Condition, actualValue: any): boolean {
    ...
    switch (operator) {
      ...
      case 'contains':
        return this.toString(actualValue, caseSensitive).includes(this.toString(value, caseSensitive));
      
      case 'not_contains':
        return !this.toString(actualValue, caseSensitive).includes(this.toString(value, caseSensitive));
      
      case 'starts_with':
        return this.toString(actualValue, caseSensitive).startsWith(this.toString(value, caseSensitive));
      
      case 'ends_with':
        return this.toString(actualValue, caseSensitive).endsWith(this.toString(value, caseSensitive));
      ...
      case 'percentage_increase':
        return this.hasPercentageIncrease(condition, actualValue, threshold || 0);
      
      case 'percentage_decrease':
        return this.hasPercentageDecrease(condition, actualValue, threshold || 0);
      
      case 'regex_match':
        return regex ? new RegExp(regex, caseSensitive ? 'g' : 'gi').test(this.toString(actualValue, caseSensitive)) : false;
      
      case 'regex_not_match':
        return regex ? !new RegExp(regex, caseSensitive ? 'g' : 'gi').test(this.toString(actualValue, caseSensitive)) : true;
  ...
  private isEqual(actualValue: any, expectedValue: any, caseSensitive = false): boolean {
  ...
  private hasChanged(condition: Condition, actualValue: any): boolean {
    ...
    const previousValue = this.extractValueFromData(condition, this.context.previousData);
  ...
  private hasPercentageIncrease(condition: Condition, actualValue: any, threshold: number): boolean {
    if (!this.context.previousData) return false;
  ...
  private hasPercentageDecrease(condition: Condition, actualValue: any, threshold: number): boolean {
    if (!this.context.previousData) return false;
    ...
    const percentageChange = ((previousNum - currentNum) / Math.abs(previousNum)) * 100;
    return percentageChange >= threshold;
  }
  ...
  private calculateGroupResult(operator: 'AND' | 'OR', results: (EvaluationResult | GroupEvaluationResult)[]): boolean {
  ...
  private generateMessage(condition: Condition, actualValue: any, passed: boolean): string {
  ...
  private generateGroupMessage(group: ConditionGroup, results: (EvaluationResult | GroupEvaluationResult)[], passed: boolean): string {
  ...
  static evaluateConditions(conditions: Condition[], context: ConditionEvaluationContext): EvaluationResult[] {
    const evaluator = new ConditionEvaluator(context);
    return conditions.map(condition => evaluator.evaluateCondition(condition));
  }

  static evaluateConditionGroups(groups: ConditionGroup[], context: ConditionEvaluationContext): GroupEvaluationResult[] {
    const evaluator = new ConditionEvaluator(context);
...
export const ConditionTemplates = {
  ...
  followersDecrease: (platform: 'facebook' | 'twitter' | 'instagram' | 'youtube', threshold: number): Condition => ({
