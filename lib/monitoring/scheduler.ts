...
export class MonitorScheduler {
  ...
  private async loadMonitors(): Promise<void> {
    try {
      ...
      this.log(
        `Error loading monitors: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async processJobs(): Promise<void> {
    ...
    const availableSlots =
      this.config.maxConcurrentJobs - this.runningJobs.size;
  ...
  private buildScrapeOptions(monitor: Monitor): ScrapeOptions {
    ...
    switch (monitor.type) {
      ...
      case "HTTPS":
        ...
        if (monitor.body) {
          options.actions = [{ type: "wait", delay: 1000 }];
        }
        break;
  ...
  private async storeCheckResult(
    ...
    try {
      ...
      this.log(
        `Error storing check result for monitor ${monitor.id}: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async triggerAlerts(
    ...
    try {
      ...
      this.log(
        `Alerts triggered for monitor ${monitor.id}: ${alertIds.join(", ")}`,
      );
      ...
      this.log(
        `Error triggering alerts: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async handleJobError(job: ScheduledJob, error: any): Promise<void> {
    ...
    if (job.retryCount >= job.maxRetries) {
      ...
      this.log(
        `Job ${job.id} failed after ${job.maxRetries} retries: ${job.lastError}`,
      );
    } else {
      job.status = JobStatus.PENDING;
      job.nextRunTime = new Date(Date.now() + this.config.retryDelay * 1000);
      this.log(
        `Job ${job.id} failed (attempt ${job.retryCount}/${job.maxRetries}), retrying in ${this.config.retryDelay}s`,
      );
    }
  }
  ...
  private async cleanup(): Promise<void> {
    ...
    try {
      ...
      const jobsToRemove = Array.from(this.jobs.entries()).filter(
        ([_, job]) =>
          (job.status === JobStatus.COMPLETED ||
            job.status === JobStatus.FAILED) &&
  ...
  private log(message: string): void {
    if (this.config.enableLogging) {
      console.log(
        `[MonitorScheduler] ${new Date().toISOString()} - ${message}`,
      );
    }
  }
}
