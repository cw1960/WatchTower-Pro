...
async function main() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const check = await prisma.monitorCheck.create({
      data: {
        ...
        responseSize: isSuccess ? Math.floor(Math.random() * 10000) + 1000 : null,
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const check = await prisma.monitorCheck.create({
      data: {
        ...
        responseSize: isSuccess ? Math.floor(Math.random() * 15000) + 2000 : null,
        errorMessage: isSuccess ? null : Math.random() > 0.5 ? 'Page not found' : 'Server error',
        checkedAt: checkTime,
        createdAt: checkTime,
      },
    })
  ...
  console.log('✅ Created alerts:', { alert1: alert1.name, alert2: alert2.name })
  ...
  console.log('✅ Created incidents:', { incident1: incident1.title, incident2: incident2.title })
  ...
  const notification1 = await prisma.notification.create({
    data: {
      ...
      content: 'Your website https://example.com is down. Status: 500 Internal Server Error.',
  ...
  const notification2 = await prisma.notification.create({
    data: {
      ...
      content: 'Your API endpoint https://api.example.com is responding slowly. Average response time: 4.2s.',
  ...
  console.log('✅ Created notifications:', { notification1: notification1.subject, notification2: notification2.subject })
