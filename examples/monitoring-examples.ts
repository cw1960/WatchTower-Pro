...
export async function runManualMonitorTest() {
  ...
  try {
    ...
    console.log(`   • Alerts Triggered: ${result.alertsTriggered?.length || 0}`);
    
    if (result.scrapeResult) {
      console.log(`   • Response Time: ${result.scrapeResult.responseTime}ms`);
      console.log(`   • Status Code: ${result.scrapeResult.data?.statusCode || 'N/A'}`);
    }
...
export async function getMonitoringStats() {
  ...
  try {
    ...
    console.log(`   • Success Rate: ${((stats.successfulChecks / stats.totalChecks) * 100).toFixed(1)}%`);
    ...
    console.log(`   • Success Rate: ${(schedulerMetrics.successRate * 100).toFixed(1)}%`);
